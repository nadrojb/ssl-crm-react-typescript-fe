...
export const InstitutionForm = ({
  ...
  const [values, setValues] = useState<InstitutionFormValues>({
    ...
    institution_type_id: initialValues?.institution_type_id
        ? String(initialValues.institution_type_id)
        : "",
    ...
    existingPrimaryContactId:
        initialValues?.contact && "id" in initialValues.contact
  ...
  const buildSubmitPayload = (): InstitutionFormSubmitPayload => {
    const contact =
        values.primaryContactMode === "existing"
            ? values.existingPrimaryContactId
                ? { id: Number(values.existingPrimaryContactId) }
                : null
            : {
              first_name: values.contactFirstName.trim(),
              last_name: values.contactLastName.trim(),
              email: values.contactEmail.trim().toLowerCase(),
              phone_number: values.contactPhoneNumber.trim() || null,
            };
    ...
    return {
      institution: {
        ...
        institution_type_id: values.institution_type_id
            ? Number(values.institution_type_id)
            : null,
        service_due_at: values.service_due_at || null,
        service_booked_at: values.service_booked_at || null,
        remedials_booked_at: values.remedials_booked_at || null,
      },
    };
  };
